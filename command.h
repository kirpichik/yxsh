...
class Command {
 public:
  Command(const std::vector<std::string>& args, const bool bkg, const std::string& in, const std::string& out, const std::string& app) : arguments(args), background(bkg), infile(in), outfile(out), appfile(app) {}
  ...
  const std::string& getApplicableFile() const {
    return appfile;
  }
...
class CommandsSet {
 public:
  ...
  const Command& operator[](int pos) const {
    return commands[pos];
  }
